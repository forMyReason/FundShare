...
### 模拟器里无法用电脑键盘输入（含小键盘数字）

1. 模拟器已连接 `adb` 时执行：`.\scripts\fix_emulator_network.ps1`（会设置 **连接实体键盘时仍允许弹出软键盘**，避免输入法不出现）。
2. 再执行（改 AVD `config.ini`，**改完后关掉模拟器再开**）：`.\scripts\enable_avd_hw_keyboard.ps1`
3. 模拟器 **设置 → 系统 → 语言和输入法 → 实体键盘**（路径因版本略不同）：开启「显示屏幕键盘」或「使用实体键盘」类选项。
4. 侧栏 **⋯ → Settings**：若有 **Send keyboard input to device**，请打开。
5. 维护页「基金代码」输入框已使用**数字键盘类型**；若数字小键盘仍不响应，先切到系统顶栏英文输入法再试。

### 维护页「按代码新增基金」报错、疑似无法联网

- 若 Snackbar 出现 **`Failed to resolve 'fund.eastmoney.com'`** / **`No address associated with hostname`**，这是 **模拟器内 DNS 解析失败**（不是 HTTP 被拦）。请依次尝试：
  1. 模拟器保持开机，在仓库根目录执行：`.\scripts\fix_emulator_network.ps1`（会关闭 **Private DNS**、并可选自检 ping）。
  2. **完全退出模拟器**后，用 `.\scripts\start_emulator.ps1` 或 `-Detach` 再开（脚本已带 `-dns-server`）。
  3. 手动打开模拟器 **设置 → 网络和互联网 → 互联网 → 网络偏好设置 / Private DNS**，设为 **关闭（Off）**。
- 本应用通过 **HTTP** 访问东方财富；Manifest 已开明文并配有 `network_security_config`。
- 若公司网络需代理，模拟器未必走系统代理，可换热点或直连环境试。
- 核心库已带 **浏览器式 User-Agent**；若仍失败，请保留完整报错文本排查。
