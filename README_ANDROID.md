...
1. 模拟器已连接 `adb` 时执行：`.\scripts\fix_emulator_network.ps1`（会设置 **连接实体键盘时不强制弹软键盘**，利于数字小键盘）。
2. 再执行（改 AVD `config.ini`，**改完后关掉模拟器再开**）：`.\scripts\enable_avd_hw_keyboard.ps1`
3. 模拟器 **设置 → 系统 → 语言和输入法 → 实体键盘**（路径因版本略不同）：关闭「**屏幕键盘**」或开启「使用实体键盘」类选项。
4. 侧栏 **⋯ → Settings**：若有 **Send keyboard input to device**，请打开。
5. 维护页「基金代码」输入框已使用**数字键盘类型**，请优先试小键盘；仍不行可暂用软键盘。
