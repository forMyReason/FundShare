...
1. 在仓库根目录执行一次（会改本机 AVD 的 `config.ini`，**改完后请关掉模拟器再开**）：
   ```powershell
   .\scripts\enable_avd_hw_keyboard.ps1
   ```
2. 模拟器侧栏 **⋯（Extended controls）→ Settings**，若有 **Send keyboard input to device / 将键盘输入发送到设备** 类选项，请打开。
3. 仍异常时，可暂用**模拟器右侧软键盘**或**长按输入框**切换输入法。

### 维护页「按代码新增基金」报错、疑似无法联网

- 本应用通过 **HTTP** 请求东方财富等接口；Manifest 已开 `usesCleartextTraffic`，并配有 `network_security_config`。
- 启动模拟器的脚本已带 **`-dns-server 8.8.8.8,8.8.4.4`**，减少「能开网页但 Python 请求失败」的 DNS 问题；**请完全退出模拟器后**再用 `.\scripts\start_emulator.ps1`（或 `-Detach`）重新启动，使新参数生效。
- 若公司网络需代理，系统代理不一定作用于模拟器，可换手机热点或在可直连公网的环境试。
- 核心库已带 **浏览器式 User-Agent**，避免部分站点拒绝 Python 默认客户端；若仍失败，把 Snackbar/报错全文记下排查。
